...
const SeccionSanciones: React.FC<SeccionProps> = ({ refreshKey, onEdit }) => {
    ...
    useEffect(() => {
        ...
        const cargarSanciones = async () => {
            try {
                const token = sessionStorage.getItem('token');
                const response = await fetch('http://localhost:3001/api/sancion', {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        const fetchSanciones = async () => {
            try {
                const res = await axios.get('http://localhost:3001/api/sancion');
                setData(res.data);
            } catch (err) {
    ...
    const handleEliminar = async (id: number, nombre: string) => {
        if (window.confirm(`¿Está seguro de eliminar la sanción de ${nombre}? Esta acción borrará el archivo físico de la resolución y restaurará el estado del árbitro.`)) {
            try {
                await axios.delete(`http://localhost:3001/api/sancion/${id}`);
    ...
    const verResolucion = (url: string) => {
        ...
        const urlCompleta = `http://localhost:3001${url}`;
