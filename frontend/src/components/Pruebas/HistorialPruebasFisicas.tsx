...
const HistorialPruebasFisicas: React.FC<HistorialProps> = ({ onBack }) => {
  ...
  const fetchHistorial = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:3001/api/pruebas-fisicas/historial');
  ...
  const verDetalles = async (prueba: any) => {
    ...
    try {
      const res = await axios.get(`http://localhost:3001/api/pruebas-fisicas/detalle`, {
  ...
  const eliminarPrueba = async (fecha: string, tipo: string) => {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/pruebas-fisicas/eliminar`, {
        params: { fecha, tipo }
      });
        ...
        <div className="space-y-6">
          {infoPrueba.url_informe && (
            <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <a 
                href={`http://localhost:3001${infoPrueba.url_informe}`} target="_blank" rel="noopener noreferrer"
